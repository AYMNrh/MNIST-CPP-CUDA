...
vector<vector<vector<double>>> max_pooling(const vector<vector<vector<double>>>& input, int pool_size) {
    ...
    int num_filters = input.size();        // Number of filters
    int input_height = input[0].size();    // Height of input feature map
    int input_width = input[0][0].size();  // Width of input feature map
    ...
    vector<vector<vector<double>>> pooled_output(num_filters, vector<vector<double>>(output_height, vector<double>(output_width, 0.0)));
    ...
    for (int k = 0; k < num_filters; ++k) {
        ...
        for (int i = 0; i < output_height; ++i) {
            for (int j = 0; j < output_width; ++j) {
                ...
                double max_val = input[k][i * pool_size][j * pool_size];
                ...
                for (int u = 0; u < pool_size; ++u) {
                    for (int v = 0; v < pool_size; ++v) {
                        ...
                        max_val = max(max_val, input[k][i * pool_size + u][j * pool_size + v]);
                    }
                }

                // Store the maximum value in the pooled output feature map
                pooled_output[k][i][j] = max_val;
            }
        }
    }
...
vector<double> flatten(vector<vector<vector<double>>>& inputs) {
    ...
    vector<double> flattened(inputs.size() * inputs[0].size() * inputs[0][0].size());
